...
const rezepteColl = collection(getFirestore(firebase), "rezepte");
...
export function RezepteProvider({ children }) {
  ...
  return (
    <Context.Provider value={[rezepte, loading, error]}>
      {children}
    </Context.Provider>
  );
}
  ...
  return useMemo(() => {
    return (rezept) => {
      ...
      if (
        kategorien &&
        kategorien.length &&
        !kategorien.some((kat) => rezept.kategorien && rezept.kategorien[kat])
      )
        return false;
      return true;
    };
...
export function useRezept(id) {
  ...
  return [
    rezepte && rezepte.find((rezept) => rezept.id === id),
    loading,
    error,
  ];
}
  ...
  return useCallback(
    async (values, images) => {
      ...
      const savedImages = await Promise.all(
        images.map((img) => (img.src ? img : saveImage(img, ref.id)))
      );
      const rezept = {
        ...values,
        images: savedImages,
...
async function saveImage(file, rezeptId) {
  ...
  const imageId = ulid();
  const ref = storage.ref().child("rezepte").child(rezeptId).child(imageId);
